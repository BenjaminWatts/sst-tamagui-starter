...
import { CognitoProviderBaseArgs, LoginResponse } from "app/features/auth/types.provider";
...
export const storeCreds = async (options: OnTokenResult) => {
    ...
    await Promise.all([
        SecureStore.setItemAsync(credsKey, JSON.stringify({email: options.email, password: options.password})),
